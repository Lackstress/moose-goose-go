...
// Bare server proxy for web proxy functionality
...
// ============================================================================
// LUNAAR HUB - Unblocked Games & Proxy
// ============================================================================

let lunaarPath = path.join(__dirname, '..', 'lunaar.org-main', 'lunaar.org-main');
try {
  const fs = require('fs');
  if (!fs.existsSync(lunaarPath)) {
    const fallback = path.join(__dirname, 'lunaar');
    if (fs.existsSync(fallback)) {
      lunaarPath = fallback;
    }
  }
...
const serveLunaarHtml = (htmlFile) => (req, res) => {
  const fs = require('fs');
  const htmlPath = path.join(lunaarPath, 'public', 'html', htmlFile);

  if (!fs.existsSync(htmlPath)) {
    return res.status(404).send('Lunaar is not installed.');
  }
  ...
  try {
    ...
    // Rewrite asset paths from ../css/ ../js/ ../media/ to /lunaar/css/ etc.
    html = html.replace(/href="\.\.\/([^"]+)"/g, 'href="/lunaar/$1"');
    html = html.replace(/src="\.\.\/([^"]+)"/g, 'src="/lunaar/$1"');
    
    // Rewrite absolute paths
    html = html.replace(/href="\/(?!lunaar|http|https|\/)/g, 'href="/lunaar/');
    html = html.replace(/src="\/(?!lunaar|http|https|\/)/g, 'src="/lunaar/');
          ...
          function fixLinks(){
            try {
              document.querySelectorAll('a[href]').forEach(function(a){
                var href = a.getAttribute('href');
                if (!href) return;
                // Map Lunaar's internal routes
                if (href === '/' || href === '/index.html') {
                  a.setAttribute('href', basePath + '/');
                } else if (href === '/science' || href === '/play') {
                  a.setAttribute('href', basePath + '/science');
                } else if (href === '/math') {
                  a.setAttribute('href', basePath + '/math');
                } else if (href === '/go') {
                  a.setAttribute('href', basePath + '/go');
                } else if (href === '/settings') {
                  a.setAttribute('href', basePath + '/settings');
                } else if (href === '/ai') {
                  a.setAttribute('href', basePath + '/ai');
                } else if (href === '/more') {
                  a.setAttribute('href', basePath + '/more');
                } else if (href.startsWith('/') && !href.startsWith(basePath) && !href.startsWith('//')) {
                  a.setAttribute('href', basePath + href);
                }
              });
            } catch(_) {}
    ...
    console.error('Failed to serve Lunaar page:', e);
    return res.status(500).send('Lunaar failed to load.');
  }
};

// Lunaar routes - map to their HTML files
app.get('/lunaar', serveLunaarHtml('index.html'));
app.get('/lunaar/', serveLunaarHtml('index.html'));
app.get('/lunaar/index.html', serveLunaarHtml('index.html'));
app.get('/lunaar/science', serveLunaarHtml('games.html'));
app.get('/lunaar/play', (req, res) => res.redirect('/lunaar/science'));
app.get('/lunaar/math', serveLunaarHtml('apps.html'));
app.get('/lunaar/go', serveLunaarHtml('go.html'));
app.get('/lunaar/settings', serveLunaarHtml('settings.html'));
app.get('/lunaar/ai', serveLunaarHtml('ai.html'));
app.get('/lunaar/more', serveLunaarHtml('more.html'));
app.get('/lunaar/new', serveLunaarHtml('new.html'));
app.get('/lunaar/forum/*', serveLunaarHtml('forum.html'));

// Lunaar CDN proxy - proxy to Parcoil's game CDN
app.use('/lunaar/cdn', (req, res) => {
  const https = require('https');
  const targetUrl = 'https://gms.parcoil.com' + req.url;
  ...
  https.get(targetUrl, (proxyRes) => {
    ...
    console.error('Lunaar CDN proxy error:', err);
    res.status(502).send('CDN proxy error');
  });
});

// Serve static Lunaar files from public directory
app.use('/lunaar', express.static(path.join(lunaarPath, 'public'), { index: false }));
...
server.listen(PORT, () => {
  ...
  console.log(`🌙 Lunaar: http://localhost:${PORT}/lunaar`);
